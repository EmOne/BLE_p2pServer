...
#define T_INT_IRQn EXTI9_5_IRQn
#define T_INT_CLK_ENABLE() __HAL_RCC_GPIOA_CLK_ENABLE()
...
//		config |= (1 << 4);	//D4 0: 3 Wire, 1: 2/4 Wire
...
void TemperatureSink_IRQHandler(void)
{
	...
	hTemperature->iTemperature_Value = (uint16_t) ((((float) le / 32.0f)
			- 256.0f) * 100.0f);
